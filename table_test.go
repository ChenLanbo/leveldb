...
func TestTableIterator(t *testing.T) {
  ...
  for i := 0; i < N; i++ {
    ...
    if DefaultComparator.Compare(iter.Key(), key) != 0 {
...
func TestMergeIterator(t *testing.T) {
  ...
  children := []Iterator{iter1, iter2}

  iter := NewMergeIterator(DefaultComparator, children)

  iter.SeekToFirst()
  for iter.Valid() {
    cnt++
    if key != nil && DefaultComparator.Compare(iter.Key(), key) <= 0 {
